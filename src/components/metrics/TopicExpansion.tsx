...
export default function TopicExpansion({ data }: { data: TopicExpansionType }) {
  ...
  const visibleNew = newTopics.slice(0, 5)
  const hiddenNew  = Math.max(0, newTopics.length - 5)
      ...
      {visibleNew.length > 0 && (
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {hiddenNew > 0 && (
              <span className="px-3 py-1.5 rounded-full text-xs font-medium text-[#6F6F78]"
